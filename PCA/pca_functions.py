...
def plot_results(n_spectra,pc_array, energies, eigenvalues, spectrum_errors, eigenval_errors):
	...
	for i in range(0,n_spectra):
		if n_spectra<=len(colours):
			p.errorbar(i+1,[eigenvalues[i]], yerr = eigenval_errors[i], marker=eigenvalmarker,color=colours[i],ls='-',ms=8)
		else:
			p.errorbar(i+1,[eigenvalues[i]], yerr = eigenval_errors[i], marker=eigenvalmarker,ls='-',ms=8)
