...
test <- sim.lc.lag(2, bins = 1024, time.lag = 0, freq.range = c(1e-4,4e-4), scale.factor = 10, shift.factor = 11)
lc1 <- data.frame(TIME = test$TIME, TIMED = test$TIMED, RATE = test$LC1.RATE, ERROR = test$LC1.ERROR, BACKV = test$LC1.RATE, BACKE = test$LC1.ERROR)
lc1$BACKV <- 0.1
lc1$BACKE <- 0.1
lc2 <- data.frame(TIME = test$TIME, TIMED = test$TIMED, RATE = test$LC2.RATE, ERROR = test$LC2.ERROR, BACKV = test$LC1.RATE, BACKE = test$LC1.ERROR)
lc2$BACKV <- 0.1
lc2$BACKE <- 0.1

lc1 <- bin.lc(lc1,200)
lc2 <- bin.lc(lc1,200)
lc2 <- lc.setOrigin(lc2, lc2$TIME[[2]])

plot(lc.plot(lc1, background = TRUE))
plot(lc.plot(lc2, background = TRUE))

cspec <- crossspec(lc1,lc2)
lagspec <- lagfrq2(cspec)

plt <- ggplot(data = lagspec) +
  geom_linerange(aes(x=FRQ,ymin=LAG-LAG.ERR, ymax=LAG+LAG.ERR)) +
  geom_hline(yintercept = 0)
plot(plt)


plot(lc.overplot(lc1, lc2, bg1 = TRUE, bg2 = TRUE))

write.table(lc1,"tables/sim_lc_1.lc",sep = ",", row.names = FALSE)
write.table(lc1,"tables/sim_lc_2.lc",sep = ",", row.names = FALSE)
